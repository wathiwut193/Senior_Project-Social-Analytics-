...
def newsreader_wordcut():
    ...
    """
    with codecs.open("news/input/1.txt",'r',encoding='utf-8') as readfile: #readfile
        data = readfile.read()
        cut = data.split('"')
        readfile.close()

    with codecs.open("news/output/1_ตัด.txt",'w+',encoding='utf-8') as write_split: #write_split
        write_split.write(''.join(cut))
        write_split.close()
    
    with codecs.open("news/output/1_ตัด.txt",'r+',encoding='utf-8') as readfile_split: #read_data
